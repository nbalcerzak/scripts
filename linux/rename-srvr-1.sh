...
if [ "$WHERE" == ph ]; then
    ...
    wget -O /etc/puppet/puppet.conf http://10.203.49.23/pub/apps/puppet/puppet.conf
    puppet agent -t
else
    rm -rf /var/lib/puppet/ssl/*
    wget -O /etc/puppet/puppet.conf http://10.185.42.42/pub/apps/puppet/puppet.conf
    puppet agent -t
fi
