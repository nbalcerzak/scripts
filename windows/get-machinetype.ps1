...
.EXAMPLE
   ...
   Get-MachineType -ComputerName (Get-Content c:\temp\servers.txt) | Format-Custom -Expand Both
.LINK
    ...
    Process
    {
        foreach ($Computer in $ComputerName) {
            ...
            try {
                ...
                # Set virtual file path
                $v_file = "c:\temp\v_servers.txt"

                # Set physical file path
                $p_file = "c:\temp\p_servers.txt"
                ...
                If ($MachineType -eq "VM") {
                  $MachineTypeInfo | Select $ComputerSystemInfo.PSComputername | Add-Content -Path $v_file
                    } else {
                        $MachineTypeInfo | Select $ComputerSystemInfo.PSComputername | Add-Content -Path $p_file
                    }
                }
            catch [Exception] {
